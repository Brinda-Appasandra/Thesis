...
def read_all_times(filepath):
    metrics = {
        'encapsulation_time': defaultdict(list),
        'encrypt_time': defaultdict(list),
        'decrypt_time': defaultdict(list),
        'decapsulation_time': defaultdict(list),
        ...
        with open(filepath, 'r') as file:
            ...
            for row in reader:
                try:
                    ...
                    metrics['encapsulation_time'][data_size].append(float(encrypt_time))
                    metrics['encrypt_time'][data_size].append(float(sign_time))
                    metrics['decrypt_time'][data_size].append(float(verify_time))
                    metrics['decapsulation_time'][data_size].append(float(decrypt_time))
...
def plot_stddev(metric_name, std_devs):
    ...
    plt.title(f'Standard Deviation of {metric_name.replace("_", " ").title()} vs Data Size for Falcon (AES-256)')
    plt.grid(True)
