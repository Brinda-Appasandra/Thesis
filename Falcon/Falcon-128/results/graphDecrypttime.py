...
# Read decrypt times from CSV file
def read_computation_times(filepath):
    ...
    try:
        with open(filepath, 'r') as file:
            ...
            for row in reader:
                try:
                    iteration, data_size, _, _, _, decrypt_time, _ = row
                    data_size = float(data_size)
                    decrypt_time = float(decrypt_time)
                    data[data_size].append(decrypt_time)
...
def save_standard_deviation(filepath, std_devs):
    try:
        with open(filepath, 'w', newline='') as file:
            ...
            writer.writerow(['Data Size', 'Standard Deviation of Decrypt Time'])
...
def plot_standard_deviation(std_devs):
    ...
    plt.ylabel('Standard Deviation of Decrypt Time (seconds)')
    plt.title('Standard Deviation of Decrypt Time for Different Data Sizes (Falcon - AES128)')
    plt.grid(True)
    plt.xlim(0, max(data_sizes) + 5000)
    plt.xticks(np.arange(0, max(data_sizes) + 5000, step=5000))

    # Save the plot as PNG
    plt.savefig('decrypt_time_stddev_plot.png')  # You can change the filename and format
    print("Plot saved as decrypt_time_stddev_plot.png")
     # Save the plot as PNG
    plt.savefig('decrypt_time_stddev_plot.png')  
    print("Plot saved as decrypt_time_stddev_plot.png")
...
if __name__ == "__main__":
    ...
    output_filepath = 'standard_deviation_decrypt_time.csv'
