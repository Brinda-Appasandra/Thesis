...
def read_computation_times(filepath):
    ...
    try:
        with open(filepath, 'r') as file:
            ...
            for row in reader:
                try:
                    iteration, data_size, _, sign_time, _, _, _ = row  # Extract sign_time
                    data_size = float(data_size)
                    sign_time = float(sign_time)
                    data[data_size].append(sign_time)
...
def save_standard_deviation(filepath, std_devs):
    try:
        with open(filepath, 'w', newline='') as file:
            ...
            writer.writerow(['Data Size', 'Standard Deviation of Sign Time'])
...
def plot_standard_deviation(std_devs):
    ...
    plt.ylabel('Standard Deviation of Sign Time (seconds)')
    plt.title('Standard Deviation of Sign Time for Different Data Sizes (Falcon - AES128)')
    plt.grid(True)
    ...
    plt.savefig('sign_time_stddev_plot.png') 
    print("Plot saved as sign_time_stddev_plot.png")

    plt.show()

if __name__ == "__main__":
    input_filepath = 'computation_times.csv'
    output_filepath = 'standard_deviation_sign_time.csv'
