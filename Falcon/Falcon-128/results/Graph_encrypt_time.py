...
# Read encryption times from CSV file
def read_computation_times(filepath):
    ...
    try:
        with open(filepath, 'r') as file:
            ...
            for row in reader:
                try:
                    iteration, data_size, encrypt_time, _, _, _, _ = row  # Extract encrypt_time
                    data_size = float(data_size)
                    encrypt_time = float(encrypt_time)
                    data[data_size].append(encrypt_time)
...
def save_standard_deviation(filepath, std_devs):
    try:
        with open(filepath, 'w', newline='') as file:
            ...
            writer.writerow(['Data Size', 'Standard Deviation of Encrypt Time'])
...
def plot_standard_deviation(std_devs):
    ...
    plt.ylabel('Standard Deviation of Encrypt Time (seconds)')
    plt.title('Standard Deviation of Encryption Time for Different Data Sizes (Falcon - AES128)')
    plt.grid(True)
    ...
    plt.savefig('encrypt_time_stddev_plot.png')  
    print("Plot saved as encypt_time_stddev_plot.png")

    plt.show()

if __name__ == "__main__":
    input_filepath = 'computation_times.csv'
    output_filepath = 'standard_deviation_encrypt_time.csv'
